...
int CpuFreq_init(const Machine* machine, CpuFreqData* data) {
   ...
   for (size_t i = 0U; i < CPUFREQ_NUM_CLUSTER_TYPES; i++) {
      ...
      CFIndex voltage_states_length = CFDataGetLength(voltage_states_ref);
      if (voltage_states_length < 8 || voltage_states_length % 8 != 0 || voltage_states_length / 8 > UINT32_MAX) {
      ...
      cluster_frequencies->num_frequencies = (uint32_t) voltage_states_length / 8;
   ...
   if (data->subscription == NULL || data->subscribed_channels == NULL)
      return -1;
...
void CpuFreq_update(CpuFreqData* data) {
   ...
   IOReportIterate(samples_delta, ^(IOReportSampleRef ch) {
      ...
      uint64_t total_residency = 0U;
      for (uint32_t i = 0U; i < state_count; i++) {
         const uint64_t residency = IOReportStateGetResidency(ch, i);
      ...
      if (total_residency > 0)
         data->frequencies[cpu_i] = average_freq / total_residency / 1000000; // Convert to MHz
